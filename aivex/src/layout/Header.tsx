...
function Header({ backendOnline }: { backendOnline: boolean }) {
  ...
  const handleProfileClick = useCallback((name: string) => {
    ...
    setProfile(name);
    setProfileMenu(false);
  }, [currentTier, setProfile, setProfileMenu, openSubscriptionAt]);
        ...
        <div className="flex items-center gap-2 leading-none">
          ...
          <div className="relative no-drag">
            <button
              onMouseDown={(e) => {
                e.stopPropagation();
                setProfileMenu((prev: boolean) => !prev);
                setSettingsOpen(false);
                setSubscriptionOpen(false);
                setProfileCreatorOpen(false);
                setThemeCreatorOpen(false);
                ...
                >
                  {[
                    ...
                    ...customProfiles.map((item) => ({
                      name: item.name,
                      desc: "Пользовательский профиль" as string | undefined,
                    })),
                  ].map((item) => {
