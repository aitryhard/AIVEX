...
const TIERS = [
  ...
  },
  {
    ...
    icon: Sparkles,
    features: [
      { text: "Неограниченное количество сообщений", icon: InfinityIcon },
...
const SubscriptionPanel = forwardRef<HTMLDivElement, { open: boolean; onClose: () => void; currentTier: string; highlightTier: string | null }>(function SubscriptionPanel(
  ...
  const setTierRef = useCallback((tierId: string, el: HTMLDivElement | null) => {
    tierRefs.current[tierId] = el;
  }, []);
      ...
      {open && (
        ...
        >
          <div className="relative">
                  ...
                  return (
                    ...
                    >
                      ...
                      <ul className={`space-y-2 ${isBelow ? "opacity-50" : ""}`}>
                        {tier.features.map((f: { text: string; icon: React.ComponentType<{ size?: number; className?: string }> }) => {
            ...
            <ScrollFade position="top" visible={canScrollUp} panelColor={uiSettings.panelColor} rounded />
            <ScrollFade position="bottom" visible={canScrollDown} panelColor={uiSettings.panelColor} rounded />
          </div>
        </motion.div>
      )}
    </AnimatePresence>
