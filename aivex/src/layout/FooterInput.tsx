...
function FooterInput() {
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent) => {
    ...
    imageItems.forEach((item) => {
      ...
      reader.onload = () => {
        setClipboardImages((prev: string[]) => [...prev, reader.result as string]);
      };
      reader.readAsDataURL(file);
    });
  }, [isFree, setClipboardImages, openSubscriptionAt]);
  ...
  const handleRemoveImage = useCallback((index: number) => {
    setClipboardImages((prev: string[]) => prev.filter((_: string, i: number) => i !== index));
  }, [setClipboardImages]);
  ...
  }, [isLoading, cancelRequest, sendMessage]);
      ...
      >
        {clipboardImages.length > 0 && (
          <div className="mb-3 flex items-center gap-2 flex-wrap">
            {clipboardImages.map((image: string, index: number) => (
              <button
                  key={index}
