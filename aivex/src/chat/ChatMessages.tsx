...
function ChatMessages({ renderMarkdown }: { renderMarkdown: (text: string, copyCode: (code: string) => void, copiedCode: string, uiSettings: UiSettings) => React.ReactElement }) {
  ...
  const filtered = messages.filter((m: Message) => !START_MESSAGES.includes(m.text));
  ...
  const visibleMessages = searchTerm
    ? sliced.filter((m: Message) => m.text?.toLowerCase().includes(searchTerm.toLowerCase()))
    : sliced;
  ...
  const hasOnlyStartMessages = messages.every((m: Message) => START_MESSAGES.includes(m.text));
  const currentStart = messages.find((m: Message) => START_MESSAGES.includes(m.text));
  ...
  return (
    ...
    >
      ...
      <ChatErrorBoundary>
        <AnimatePresence>
          {visibleMessages.map((item: Message, index: number) => (
              <MessageBubble
                key={item.id ?? index}
                item={item}
                index={index}
      ...
      <AnimatePresence>
        {isLoading && !messages.some((msg: Message) => msg.type === "audio-status") && (
          <motion.div
    ...
    if (this.state.error) {
      return (
        <div className="px-5 py-8 text-center">
          <p className="text-sm text-red-400/60">Ошибка отображения сообщений</p>
          <button
            onClick={() => this.setState({ error: null })}
            className="mt-3 px-4 py-2 rounded-xl bg-white/10 border border-white/10 text-xs text-white/60 hover:bg-white/15 transition"
          >
            Попробовать снова
          </button>
        </div>
