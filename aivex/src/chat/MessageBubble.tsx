...
import type { Message, UiSettings } from "../types";

function MessageBubble({ item, index, renderMarkdown }: { item: Message; index: number; renderMarkdown: (text: string, copyCode: (code: string) => void, copiedCode: string, uiSettings: UiSettings) => React.ReactElement }) {
        ...
        >
          ...
          {images.length > 0 && (
            <div className={"flex gap-2 flex-wrap" + (item.text ? " mt-3" : "")}>
              {visibleImages.map((image: string, imgIndex: number) => (
                <button
                  key={imgIndex}
